...
int main()
{
    ...
    int rh = zmq_setsockopt(receiver , ZMQ_SUBSCRIBE , "hint>" , 5);
    ...
    while(1)
    {
       ...
       if (!cin)/// (Guesses[size] < 1000 || Guesses[size] > 9999) check if input is int
        ...
        hint[3] = result [3];
        cout << "hint gegeven" << hint << endl;
        rc = zmq_send (sender , "hint" ,  30 , 0);

        std::stringstream ht;
        ht<<"hint!"<<hint<<"given hint";
        std:: string h = ht.str();
        rc = zmq_send (sender , h.c_str(),  h.size() , 0);
